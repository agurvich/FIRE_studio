...
def raytrace_metal_column_compute(
    gas_x,gas_y,gas_z,
    gas_hsml,gas_mass,
    ...
    x=np.concatenate([star_x,gas_x])
    y=np.concatenate([star_y,gas_y])
    z=np.concatenate([star_z,gas_z])
    hsml=np.concatenate([star_hsml,gas_hsml])
    mass=np.concatenate([np.zeros(star_hsml.size),gas_mass])
    ...
    ## now sort these in z (this is critical!)
    ##  get sort indices
    s=np.argsort(z)[::-1]; ## reverse for metal column
    ...
    curpath = os.path.realpath(__file__)
    curpath = curpath[:len("utils")+curpath.index("utils")] #split off this filename
    exec_call=os.path.join(curpath,'stellar_utils/c_libraries/RayTrace_RGB_metal_column/raytrace_rgb.so')
    routine=ctypes.cdll[exec_call];
    ...
    out_cast = ctypes.c_float*N_pixels
    out_0=out_cast() ## mass map
    out_cast = ctypes.c_float*N_p
    out_1=out_cast() ## metal column for each particle
    ...
    routine.raytrace_rgb( 
        ...
        ctypes.c_int(Xpixels),ctypes.c_int(Ypixels), ## output shape
        ctypes.byref(out_0), ## mass map
        ctypes.byref(out_1) ) ## metal column for each star

    ## now put the output arrays into a useful format 
    #out_0 = np.copy(np.ctypeslib.as_array(out_0));
    #out_0 = out_0.reshape([Xpixels,Ypixels]);

    #metal_columns = np.zeros(N_p)
    #for i in range(N_p):
        #metal_columns[i] = out_i[i]
    out_1 = np.copy(np.ctypeslib.as_array(out_1));

    unsort_metal_columns = np.zeros(N_p)
    #### undo the sort
    unsort_metal_columns[s] = out_1
    unsort_metal_columns = unsort_metal_columns[:star_x.size] ## stars were the first N_star many
    #
    return unsort_metal_columns
