;;
;; companion function to "luminosity_to_magnitude" -- consider that the master
;;
;;
;; function to take the input magnitude of a given band and return the 
;;    corresponding luminosity, for the appropriate bands and units as 
;;    listed below
;;
...
;;		default : bolometric
;;
;; VEGA or AB keywords tell it whether the input is either vega or ab magnitudes
;;		default : VEGA for UBVRIJHK (Johnsons UBVRI, Cousins JHK), AB for ugriz  
;;
;; L_NU or NU_L_NU tell it whether the output is in specific luminosity L_NU (e.g. 
;;   erg/s/Hz), or NU_L_NU (e.g. erg/s)
;;		default : NU_L_NU
;;
;;
;;
function magnitude_to_luminosity, M, $
...
L_of_M = nulnu_sun_band(i_BAND) * 10^(-0.4 * DOUBLE(M - magnitude_zero_point))
;; now just convert to the appropriate units
if (keyword_set(L_NU)) then L_of_M = L_of_M / nu_eff(i_BAND);
if (keyword_set(NU_L_NU)) then L_of_M = L_of_M;

if (keyword_set(UNITS_SOLAR_BOL)) then return, L_of_M/l_bol_sun;
if (keyword_set(UNITS_SOLAR_BAND)) then begin
	if (keyword_set(L_NU)) then return, L_of_M/lnu_sun_band(i_BAND);
	if (keyword_set(NU_L_NU)) then return, L_of_M/nulnu_sun_band(i_BAND);
	return, L_of_M/nulnu_sun_band(i_BAND);
endif
if (keyword_set(UNITS_CGS)) then return, L_of_M;
