...
def readsnap(sdir,snum,ptype, snapshot_name='snapshot', extension='.hdf5', h0=0,cosmological=0, skip_bh=0,
    flag_istellmass=0,four_char=0, header_only=0, loud=0):
	...
	if npart[ptype]>0:
	  nR=nL + npart[ptype]
          ...
          if (ptype==4) and (flag_sfr>0) and (flag_stellarage>0):
              ...
              if flag_istellmass:
                  istellmass[nL:nR]=input_struct[bname+'InitialMasses']
          if (ptype==5) and (skip_bh==0):
    ...
    if (ptype==4):
        ...
        if flag_metals>0:
          ret_dict['z'] = metal
        if flag_istellmass:
          ret_dict['imass']=istellmass
          ret_dict['flag_istellmass']=flag_istellmass
        return ret_dict
    if (ptype==5) and (skip_bh==0):
